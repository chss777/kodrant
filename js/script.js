...
function toPlay() {
    if  (localStorage.getItem('userlist').length < 5) {
        localStorage.setItem('userlist', localStorage.setItem('userlist', JSON.stringify(list)));
    }
    ...
    let newlist = JSON.parse(storage);
    ...
    // if (slovo.length > 11) {
    //     page.style.fontSize = '1vw';
    // } else {
    //     page.style.fontSize = '1.5vw';
    // }
...
function ultimate(a, b) {
    ...
    if (r1 === 3) {
        warn(`Вы победили со счётом  ${r1}:${r2} !`);
        location.reload();
    } else if (r2 === 3) {
        warn(`Вы проиграли со счётом  ${r1}:${r2}`);
        location.reload();
    }

    setTimeout(() => {
        mode1.addEventListener('hidden.bs.modal', toPlay());
        letter.focus();
    }, 0);
}
...
function check(simv, hidden) { //  'hidden'='slovo'
    let total = "";
    let tab = word.parentElement; // <table>

    for (let i = 0; i < hidden.length; i++) {
        if (hidden[i] === simv) {
            tab.rows[0].cells[i].innerHTML = simv;
        }
        tab.rows[0].cells[i].innerHTML ? (total += tab.rows[0].cells[i].innerHTML) : (total += '#');
    }

    if (total === slovo) {
        ultimate(total, slovo);
    }
    attempt -= 1;
...
function warn(text) {
    ...
    let close = document.querySelector('#attention button');
    let myevent = new Event('click');
    setTimeout(() => {
        document.addEventListener('click', () => close.dispatchEvent(myevent));
    }, 0);
    close.addEventListener('click', () => letter.focus());
}
...
function openLetter() {
    word.addEventListener('click', view);
}

function view(e) {
    let n = e.target.cellIndex;
    word.cells[n].innerHTML = slovo[n];
    word.removeEventListener('click', view);
